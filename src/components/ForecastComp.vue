    ...
    <div class="w-[600px] h-auto max-h-96 bg-sky-600 bg-opacity-50 rounded-lg shadow-md text-white">
        <div class="flex flex-col w-full h-full p-5">
            ...
            <div v-if="data" class="flex flex-col grow">
                <div v-for="(day, index) in data.forecast.forecastday" :key="day.date" :class="['flex justify-between items-center grow px-5 hover:bg-sky-700 transition-all', index < data.forecast.forecastday.length - 1 ? 'border-b border-white' : '']">
                    ...
                    <div>
                        ...
                        <span class="text-gray-300">{{ isCelsius ? day.day.maxtemp_c : day.day.maxtemp_f }}&deg;</span>
                    </div>
                </div>
            </div>
        </div>
